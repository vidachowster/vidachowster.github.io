...
function checkPose(prediction, video) {
    ...
    if (prob > 0.8 && !soundActive) {
        ...
        switch(poseNumber) {
            case '1':
                if (time >= 10.0 && time <= 12.3 && !poseState.triggered) {
                    triggerSound(poseState);
                }
                break;
            case '2':
                if (time >= 16.5 && time <= 17.5 && !poseState.triggered) {
                    triggerSound(poseState);
                }
                break;
            case '3':
                if (time >= 16.7 && time <= 17.5 && !poseState.triggered) {
                    triggerSound(poseState);
                }
                break;
            case '4':
                if (time >= 18.0 && time <= 19.4 && !poseState.triggered) {
                    triggerSound(poseState);
                }
                break;
            case '5':
...
async function playInstructionVideo() {
    ...
    if (model) {
        ...
        console.log("https://teachablemachine.withgoogle.com/models/xUqxPnqAX/");
    }
}
